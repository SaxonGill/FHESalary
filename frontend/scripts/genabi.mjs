...
let abiSource;
if (deployLocalhost) {
  abiSource = deployLocalhost.abi;
} else if (deploySepolia) {
  abiSource = deploySepolia.abi;
} else {
  console.warn(
    `${line}No deployments found. Generating placeholder ABI and zero addresses for both networks.${line}`
  );
  abiSource = [];
}
